...
class MainTests(unittest.TestCase):
    ...
    def test_help_is_human_friendly(self) -> None:
        ...
        self.assertIn("gcal", output)
        self.assertIn("features:", output)
        self.assertIn("authorize a Google account and save or refresh its preset", output)
        self.assertIn("create, list, delete, reschedule, and export events", output)
        self.assertIn("# gcal auth <client_secret_path>", output)
        self.assertIn('# gcal <preset> ls 5 | gcal <preset> d <event_id> | gcal <preset> r <event_id> "<start>" "<end>"', output)
        self.assertIn("gcal 1 ls 5", output)
        self.assertIn('gcal 1 r abc123 "2026-03-11 10:00:00" "2026-03-11 11:00:00"', output)
